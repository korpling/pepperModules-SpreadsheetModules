...
public class SpreadsheetImporterProperties extends PepperModuleProperties {
	...
	public SpreadsheetImporterProperties() {
		...
		addProperty(new PepperModuleProperty<>(PROP_ANNO_REFERS_TO, String.class, "Defines which annotation tier refers to which primary text tier, therefor the annotation tier name is followed by the name of the tier, that holds the primary text in square brackets. A possible key-value set could be: key='annoPrimRel', value='anno1[tok1]' (key: 'annoPrimRel', default is 'null').", null, false));
